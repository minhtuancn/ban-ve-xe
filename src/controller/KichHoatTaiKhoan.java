...
public class KichHoatTaiKhoan extends HttpServlet {
	...
	protected void doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String mes = null;
		System.out.println("KichHoatTaiKhoan: "+ maOTP + " - "+ maOTPCheck);
		if (maOTPCheck.equalsIgnoreCase(maOTP)) {
			if(taiKhoanDAO.kichHoatTaiKhoan(((KhachHangThuongXuyen)kh).getTaiKhoan().getIdTaiKhoan()))
			response.sendRedirect(DuongDan.KIEM_TRA_VE_SV);
			else response.sendRedirect(DuongDan.TRANG_CHU);
		}else{
			mes = "Sai mã OTP, vui lòng kích hoạt lại!!";
			request.setAttribute("mes", mes);
			request.getRequestDispatcher("/jsp/KichHoatTaiKhoan.jsp").forward(request, response);
		}
		}else {
