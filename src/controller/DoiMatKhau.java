...
public class DoiMatKhau extends HttpServlet {
	...
	protected void doAction(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		KhachHang khachHangDMK = (KhachHang) request.getSession().getAttribute("khachHang");
		String passKH =((KhachHangThuongXuyen) khachHangDMK).getTaiKhoan().getMatKhau();
		long upDate = -1;
		
		if(pass_old.equals(passKH)){
			if(pass.equals(re_pass)){
			 upDate = taiKhoanDAO.upDateMatKhau(pass);
			response.sendRedirect(DuongDan.KIEM_TRA_THONG_TIN);
		}else {
